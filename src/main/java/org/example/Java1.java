...
public abstract class Java1 extends JFrame
    ...
    HashMap<String, AudioInputStream> audioObjects = new HashMap<>();
    BufferedImage bi;
    JPanel pane;
    Graphics2D g2;
    ...
    /** Hides the cursor by setting its texture to an empty image */
    ...
    /**
     * Reverts the cursor to is default texture, thus making it visible if hideCursor() has been run
     */
    ...
    /**
     * Draws a rectangle filled with the specified color with it's left corner at the given x and y
     * coordinates
     *
    ...
    /**
     * Draws a rectangle outlined by the specified color with it's left corner at the given x and y
     * coordinates
     *
     * @param x the x value for the upper left corner
     * @param y the y value for the upper left corner
     ...
     * @throws IOException if the file is not found or able to be read
     */
    ...
    /**
     * Writes the given text to screen, but rotated 180 degrees has the given colour and position,
    ...
    /**
     * Rotates the screen around a point
     *
     ...
     *
     * @param color the color to flood the screen with
     */
    ...
    public void setSize(int w, int h) {
        ...
        WIDTH = w;
        HEIGHT = h;
        // calculate the new size of the Graphics2D component
        width = WIDTH - 15;
        height = HEIGHT - 40;
    ...
    /**
     * Constructor of class, runs init()
     *
     * @see #init()
     */
    public Java1() {
    ...
    private boolean isInitialised(Field field) {
        // There is no difference between Field.getAnnotations and Filed.getDeclaredAnnotations
        // The former simply calls the latter
        final Annotation[] annotations = field.getDeclaredAnnotations();

        // Since many libraries have their own "Nullable" annotations we just check the name
        // Note: The annotations will only be detected if they are annotated with @Retention(RetentionPolicy.RUNTIME)
        if (Arrays.stream(annotations).anyMatch(a -> a.annotationType().getName().endsWith("Nullable")))
            return true;

        // If the field is null we consider it uninitialised
        try {
            return field.get(this) != null;
        } catch (IllegalAccessException unreachable) { return true; }
    }

    protected boolean skipInitialisationCheck = false;

    protected static final long warningInterval = 5000; // ms
    private Long lastWarned;

    private boolean isSubclassInitialised() {
        if (skipInitialisationCheck) return true;
        if (lastWarned == null) lastWarned = System.currentTimeMillis();

        final Class<? extends Java1> subclass = this.getClass();
        // This includes the fields from all superclasses
        final Field[] fields = subclass.getFields();

        final List<Field> uninitialised = Arrays.stream(fields).filter(field -> !isInitialised(field)).toList();

        if (System.currentTimeMillis() - lastWarned > warningInterval) {
            System.out.print("Warning: Some variables are not initialised: ");
            for (final Field field: uninitialised)
                System.out.print("`" + field.getName() + "`, ");
            System.out.println();

            lastWarned += warningInterval;
        }

        return skipInitialisationCheck = uninitialised.isEmpty();
    }
    ...
    /**
     * runs the loop figure whenever an event triggers. the timer constantly running triggers this
     * event, giving us a stable framerate
     */
    @Override
    public void actionPerformed(ActionEvent e) {
        if (!isSubclassInitialised()) return;
        loop();
        repaint();
    }
    ...
    public class Turtle {
        public double x, y;
        private final Stack<Double> xList = new Stack<>();
        private final Stack<Double> yList = new Stack<>();
         ...
         * @param angle the initial angle of the turtle
         */
        ...
        public void move(int distance) {
            // calculate the new x and y values taking into account current x, y and angle
        ...
        /**
         * Save x and y positions to the stack to be recovered later
         *
         * @see #pop()
         */
        public void push() {
        ...
        public void LString(String string, double angle, int length) {
            for (int i = 0; i < string.length(); i++) {
                switch (string.charAt(i)) {
                    case 'F', 'G' -> move(length); // move forward and draw line
                    case 'f' -> moveX(length); // "jump" forward without drawing a line
                    case '+' -> turnLeft(angle); // turn left
                    case '-' -> turnRight(angle); // turn right
                    case '[' -> push(); // save position to stack
                    case ']' -> pop(); // recover position from stack
                }
            }
        }
         ...
         * modifies it according to the specified rules. The iterations variable defines how many
         ...
         * @param rules A list of In, Out; where every occurrence of in becomes the out string.
        ...
        public void Fractal(String axiom, String rules, int iterations, double angle, int length) {
            ...
            for (int i = 0; i < iterations; i++) {
                ...
                // for every character, check if it's in the indexes of _rules, otherwise we ignore
                // it
                ...
                for (int u = 0; u < currentLen; u++) {
                    for (int r = 0; r < _rules.length; r += 2) {
                        if ((String.valueOf(sys.charAt(u))).equals(_rules[r])) {
                            newSys.append(_rules[r + 1]);
                        }
                    }
                }
                sys = newSys.toString();
